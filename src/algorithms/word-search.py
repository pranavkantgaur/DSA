...
class Solution:
    ...
    def helper(self, board, word, letter_index, start_pos):
        ...
        if letter_index == len(word) - 1:
            ...
            result = self.helper(board, word[1:], 0, start_pos = [start_pos[0] + 1, start_pos[1]]) or self.helper(board, word[1:], 0, start_pos = [start_pos[0] - 1, start_pos[1]]) or self.helper(board, word[1:], 0, start_pos = [start_pos[0], start_pos[1] + 1]) or self.helper(board, word[1:], 0, start_pos = [start_pos[0], start_pos[1] - 1])            
    ...
    def exist(self, board, word):
        ...
        for row in range(m):
            for col in range(n):
                if self.helper(board, word, i, [row, col]):
                    return True
        return False
    '''
    def exist(self, board, word):
